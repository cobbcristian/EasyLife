...
/** Home “Amenities” destination — list all bookable venues for the community. */
export default async function MemberAmenitiesPage() {
  ...
  const isCondo = communityIsResidentialHoa(session.communityId);
  ...
  return (
    <div className="min-h-screen bg-white font-[family-name:var(--font-poppins)] text-ink">
      <div className="mx-auto max-w-lg px-4 pb-28 pt-[max(0.75rem,env(safe-area-inset-top))]">
        ...
        <h1 className="text-[22px] font-semibold tracking-[-0.02em]">Amenities</h1>
        <p className="mt-1 text-sm text-grey">
          {isCondo
            ? "Tennis, pool, and building amenities — tap one to reserve."
            : "Courts, spa, and facilities — tap for details, then reserve."}
        </p>
        {amenities.length === 0 ? (
          <div className="mt-8 rounded-xl bg-[#F7F8FA] p-5 text-center">
            <p className="text-sm font-semibold text-ink">No amenities listed yet.</p>
            <a
