      ...
      <div className="mx-auto flex min-h-screen w-full max-w-lg flex-col px-4 py-6">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            aria-label={t("Toggle spoken replies")}
            title={t("Spoken confirmations")}
          >
            ...
            messages.map((m, i) => (
              <div
                ...
                className={`rounded-2xl px-4 py-3 text-sm ${
                  m.role === "user"
                    ? "ml-8 bg-[var(--mvp-blue)] text-white"
                    : "mr-8 border border-[#e8ebf0] bg-[#fafbfc] text-ink"
                }`}
              >
        ...
        <form
          className="fixed inset-x-0 bottom-0 border-t border-[#eceff3] bg-white/95 px-4 py-3 backdrop-blur"
          onSubmit={(e) => {
            e.preventDefault();
            void send();
          }}
        >
          <div className="mx-auto flex max-w-lg gap-2">
            ...
            <input
              ...
              className="h-11 flex-1 rounded-2xl border border-[#e4e8ee] px-4 text-sm outline-none focus:border-[var(--mvp-blue)]"
            />
            <button
              type="submit"
              disabled={busy || !input.trim()}
              className="h-11 rounded-2xl bg-[var(--mvp-blue)] px-4 text-sm font-semibold text-white disabled:opacity-50"
            >
              {busy ? t("…") : t("Send")}
            </button>
          </div>
